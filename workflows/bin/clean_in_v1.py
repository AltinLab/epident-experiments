#!/usr/bin/env python
"""
Generate a cleaned epitope dataset (IN-V1)
...
Additionally, each protein is annotated with its pre-C-to-S substitution sequence for later key joining with focal proteins
...
def process_fasta_into_df(fasta_path):
    protein_df = fasta_to_polars(fasta_path, desc_as_name=True).rename(
        {"name": "raw_protein_id"}
    )
...
def process_epitope_data(metadata, z_score, pre_sub_peptides):
...
if __name__ == "__main__":
    ...
    epitope_dat = process_epitope_data(in1_metadata, z_score, pre_sub_peptides)
    ...
    ne_only_protein_df.write_parquet(output_path / "in_v1_ne_proteins.parquet")
    epitope_df.write_parquet(output_path / "in_v1_epitopes.parquet")
    full_protein_df.write_parquet(output_path / "in_v1_proteins.parquet")
